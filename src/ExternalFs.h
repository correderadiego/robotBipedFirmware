...
{
private:
	static void createFile(File fp, int fileSize, unsigned char* buf, int bufferSize);

public:
	//! @brief Chunk size of external EEPROM (bytes)
	inline static const int EEPROM_CHUNK_SIZE() { return 32; }
	...
	inline static const int SLOT_END()   { return MOTION_FILE_SIZE / EEPROM_CHUNK_SIZE(); }
